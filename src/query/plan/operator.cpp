...
bool addHistoryVertex(query::VertexAccessor &current_vertex_,history_delta::historyContext &historyContext_,std::list<TypedValue> &history_add_,ExecutionContext &context,bool edge_expand){
  ...
  bool history_expand_flag=true;
  if(!edge_expand){
    if(context.search_gid!=-1){
      auto get_prop_value=current_vertex_.GetProperty(storage::View::OLD, context.search_property).GetValue();
      if(get_prop_value!=context.search_value){
        history_expand_flag=false;
        delete_flag=true;
      }
    }
  }
  ...
  if(!history_expand_flag) return delete_flag;
  ...
  if(need_deleted_flag){
    ...
    for(auto gid_delta_:gid_history_deltas_){
      ...
      auto in_edges=current_vertex_.getInEdges();
      auto out_edges=current_vertex_.getOutEdges();
      for(int i=0;i<gid_history_deltas_.size();i++){
        auto gid_delta_=gid_history_deltas_[i];
        if(i==0){
          current_vertex1=context.db_accessor->CreateAnchorVertex(gid,gid_delta_,in_edges,out_edges);
        }else {
          current_vertex1=context.db_accessor->CreateHistoryVertexFromKV(current_vertex1,gid_delta_,historyContext_);
        }
        history_flag=true;
...
bool addHistoryVertex2(query::VertexAccessor &current_vertex_,history_delta::historyContext &historyContext_,history_delta::historyContext &historyContext2,std::list<TypedValue> &history_add_,ExecutionContext &context,bool edge_expand){
  auto gid=current_vertex_.Gid().AsUint();
  auto obj_ts=current_vertex_.transaction_st();//uint64_t transaction_st
  auto obj_te=current_vertex_.tt_te();//std::numeric_limits<uint64_t>::max();
  bool delete_flag=false;
  //判断是否需要删除当前数据库的节点
  if(!history_delta::TemporalCheck(obj_ts,obj_te,historyContext2.c_ts,historyContext2.c_te,historyContext2.types)){//删除当前数据库中的节点
    delete_flag=true;
  }
  if(obj_ts>=obj_te) delete_flag=true;
  ...
  bool history_flag=false;
  //dead info
  auto [need_deleted_flag,dead_deltas]=context.db_accessor->GetHistoryDelta()->getDeadInfo(current_vertex_.getDeltas(),historyContext2.c_ts, historyContext2.c_te,historyContext_.types,true);
  for (auto index:dead_deltas){
    current_vertex1=context.db_accessor->CreateHistoryVertexFromDelta((current_vertex_).impl_,index,historyContext_);
    history_flag=true;
    auto values=TypedValue(current_vertex1);
...
void pull_nodes_current_history(ExecutionContext &context,VertexAccessor current_vertex,uint64_t obj_ts,uint64_t obj_te,TypedValue current_edge,std::list<std::pair<TypedValue,TypedValue>> &history_add_,history_delta::historyContext &historyContext_) {
  ...
  if(!before_flag){//index查找顶点，没有遍历得到所有顶点的历史节点，重新获取
    std::cout<<"need get from kv\n";
    std::list<TypedValue> history_add;
    addHistoryVertex2(current_vertex,historyContext_,historyContext2,history_add,context,true);
  }
  auto vertex_vector=context.db_accessor->FindHistoryVertex(gid,historyContext_.c_ts,historyContext_.c_te);
  if(vertex_vector){
    std::cout<<"do not get to vertex \n";
    for(auto vertex:*vertex_vector){
      auto vertex1=*vertex;
      auto values=TypedValue(vertex1);
      if(tt_ts<=obj_te&obj_ts<=tt_te){//节点 边
